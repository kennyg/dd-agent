...
class Processes(ResourcePlugin):
    ...
    def describe_snapshot(self):
        return SnapshotDescriptor(1,
                                  SnapshotField(
                                      "user", 'str', aggregator=agg.append, temporal_aggregator=agg.append),
    ...
    def _get_proc_list(self):
        ...
        try:
            ...
            if process_exclude_args:
                ...
                ps_arg = 'auxww'
            ps = subprocess.Popen(
                ['ps', ps_arg], stdout=subprocess.PIPE, close_fds=True).communicate()[0]
        except Exception, e:
    ...
    def _parse_proc_list(self, processes):
        ...
        PSLine = namedtuple(
            "PSLine", "user,pid,pct_cpu,pct_mem,vsz,rss,tty,stat,started,time,command")
